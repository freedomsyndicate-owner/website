...
# --- FREEDOM SYNDICATE: GOLD PREDATOR V6 (PONOROGO VERSION) ---
...
def get_gold_price():
    # Mengambil data XAU/USD asli (Forex) lewat Yahoo Finance
    url = "https://query1.finance.yahoo.com/v8/finance/chart/XAUUSD=X?interval=1m&range=1d"
    headers = {'User-Agent': 'Mozilla/5.0'}
    try:
        res = requests.get(url, headers=headers, timeout=10).json()
        data = res['chart']['result'][0]['indicators']['quote'][0]
        curr = res['chart']['result'][0]['meta']['regularMarketPrice']
        d_open = res['chart']['result'][0]['meta']['previousClose'] # Daily Open Approach
        return {'last': float(curr), 'open': float(d_open)}
    except: return None

def monitor():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🦅 DOMS V6 PONOROGO: ONLINE.")
    last_signal_time = 0
    ...
    while True:
        ...
        if d:
            ...
            # SET WAKTU KE WIB (PONOROGO)
            now_dt = datetime.now() 
            m, h = now_dt.minute, now_dt.hour

            # --- QUARTERLY THEORY (LOGIC WIB) ---
            ...
            # --- MACRO WINDOWS WIB (Sesuaikan dengan Sesi Gold) ---
            # Contoh: 14:00-15:00 (London Open), 19:00-21:00 (NY Open)
            is_macro = (14 <= h <= 15) or (19 <= h <= 21)
            macro_status = "⚡ MACRO ACTIVE" if is_macro else "💤 OFF-MACRO"

            side, setup = "", ""
            # Logic: Premium/Discount berdasarkan Harga XAUUSD Asli
        ...
        time.sleep(20) # Cek tiap 20 detik biar akurat
